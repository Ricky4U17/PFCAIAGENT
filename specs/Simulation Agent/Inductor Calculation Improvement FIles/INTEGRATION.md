...
| `PFC_Inductor_Engine_Equations.pdf` | Step-by-step equations (E1–E61) for every computed quantity + variable glossary; transcribed from the code. | Docs / design review |
| `gen_equations_pdf.py` | Regenerates the equation PDF (keep with the code so doc and engine stay in sync). | Docs tooling |
