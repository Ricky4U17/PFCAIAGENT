...
# ============================ TITLE ============================
story.append(Paragraph("PFC Boost Inductor — Production Engine Equation Reference", st_t))
story.append(Paragraph(
    "Step-by-step equations for every quantity computed by our production backend "
    "<b>step7_magnetic_calc.py</b> (Phase-1 v10). Transcribed from the code as of 2026-06-10. "
    "Counterpart record to the Simulation-Agent's PFC_Inductor_Engine_Equations.pdf, kept for "
    "side-by-side comparison before any merge.", st_sub))
...
EQ("k(H) = DB.get_k_bias(material, H<sub>Oe</sub>)&nbsp;&nbsp;[&ndash;]",
   ...
   "Fast inner waveform loop uses an EDGE-family closed-form fallback "
   "k(H) = min(1, [1/(0.01 + 9.202&times;10<super>&minus;10</super> H<super>3.044</super>)]/100). "
...
EQ("P<sub>Cu</sub>(T) = I<sub>&phi;,rms</sub><super>2</super>&middot;R<sub>DC</sub>(T) + I<sub>hf,rms</sub><super>2</super>&middot;R<sub>AC</sub>(T)",
   ["R<sub>AC</sub>(T) = R<sub>DC</sub>(T)&middot;(R<sub>AC</sub>/R<sub>DC</sub>); reported at T<sub>core</sub>, 25 &deg;C and 100 &deg;C"])
...
story.append(tbl)
story.append(Spacer(1, 8))
B("<i>Honest status note. Strengths vs. the Simulation-Agent reference engine: temperature-aware "
  "DB core-loss and DC-bias curves (E10, E25), a physics-based Dowell-proximity R<sub>AC</sub> (E23) "
  "rather than a flat 1.15, a two-node hotspot thermal model (E48&ndash;E49), and ferrite gap fringing "
  "(E50). Open items: the inner waveform loop's k(H) fallback is EDGE-calibrated (route non-EDGE "
  "through DB.get_k_bias); the per-V<sub>in</sub> screening table (S13) is single-point and is anchored "
  "to the waveform result on the Review page; R<sub>AC</sub>, thermal and bias retention are still "
  "first-order until FEA/bench data is attached. All other quantities are exact arithmetic on the "
  "DB + designer-selected geometry/wire.</i>")

out_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                        "PFC_Inductor_OurEngine_Equations.pdf")
doc = SimpleDocTemplate(out_path, pagesize=letter, topMargin=16*mm, bottomMargin=15*mm,
                        leftMargin=16*mm, rightMargin=16*mm,
                        title="PFC Boost Inductor — Production Engine Equation Reference",
