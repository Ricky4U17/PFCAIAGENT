...
def test_phase3_advisory_nodes_execute_in_main_flow():
    graph = build_graph()
    state = build_initial_state("phase3_flow", _sample_intake())
    ...
    assert "pcb_floorplanning_advisory" in state.get("step_results", {})
    assert "cad_thermal_integration_advisory" in state.get("step_results", {})
    assert "magnetic_fea_advisory" in state.get("step_results", {})
    assert "pcb_floorplanning_results" in state
    assert "cad_thermal_integration_results" in state
    assert "magnetic_fea_results" in state
