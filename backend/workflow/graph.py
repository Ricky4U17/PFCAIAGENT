...
WAIT_CHANNELS   = "awaiting_channel_count"         # new gate: phase count for interleaved
...
def topology_specific_intake_node(state):
    ...
    if state.get("last_completed_step") == "topology_specific_intake" and state.get("waiting_at") != WAIT_TOPOLOGY_SPECIFIC:
        state["pending_step"] = "input_processing"
        return state
...
def controller_selection_hitl_node(state):
    # Auto-advance if controller mode was already chosen AND feedback not targeting this gate.
    if state.get("selected_controller_mode") and state.get("waiting_at") != WAIT_CONTROLLER:
        state["pending_step"] = WAIT_CHANNELS
        return state
    feedback = state.get("human_feedback", {})
    if feedback.get("approved", False):
        chosen_mode = (feedback.get("controller_mode")
                       or state["controller_strategy"]["recommended_controller_mode"])
        state["selected_controller_mode"]                   = chosen_mode
        state["controller_strategy"]["selected_mode"]       = chosen_mode
        state["last_completed_step"]                        = "controller_selection"
        state["human_feedback"]                             = {}   # consumed
        state["pending_step"]                               = WAIT_CHANNELS
    else:
...
INTERLEAVED_TOPOLOGIES = {"interleaved_boost_ccm", "totem_pole_interleaved_ccm"}

def channel_selection_node(state):
    """Determine whether the topology requires a channel-count gate."""
    topo = state.get("selected_topology", "")
    state["current_step"] = "channel_selection"
    if topo in INTERLEAVED_TOPOLOGIES:
        state["pending_step"] = WAIT_CHANNELS
    else:
        # Non-interleaved: default to 1 phase, skip designer input
        state["selected_channels"] = 1
        state["pending_step"]      = "topology_specific_intake"
    return state


def channel_selection_hitl_node(state):
    """Designer confirms channel count (2/3/4) for interleaved topologies."""
    # Auto-advance if channels already selected and not targeting this gate.
    if state.get("selected_channels") and state.get("waiting_at") != WAIT_CHANNELS:
        state["pending_step"] = "topology_specific_intake"
        return state
    feedback = state.get("human_feedback", {})
    if feedback.get("approved", False):
        n_channels = int(feedback.get("n_channels", 3))
        state["selected_channels"] = n_channels
        state["last_completed_step"] = "channel_selection"
        state["human_feedback"]      = {}   # consumed
        state["pending_step"]        = "topology_specific_intake"
    else:
        state["pending_step"] = WAIT_CHANNELS
...
def route_after_channel_selection_hitl(state):
    return state.get("pending_step", WAIT_CHANNELS)


def wait_channels_node(state):
    state["current_step"] = WAIT_CHANNELS
    state["waiting_at"]   = WAIT_CHANNELS
    return state
...
def build_graph():
    ...
    nodes = [
        ...
        ("channel_selection",               channel_selection_node),
        ("channel_selection_hitl",          channel_selection_hitl_node),
        (WAIT_CHANNELS,                     wait_channels_node),
    ...
    g.add_conditional_edges("topology_specific_intake", route_after_topology_specific_intake, {"input_processing":"input_processing", WAIT_TOPOLOGY_SPECIFIC:WAIT_TOPOLOGY_SPECIFIC})
    g.add_edge("controller_selection", "controller_selection_hitl")
    g.add_conditional_edges(
        "controller_selection_hitl", route_after_controller_selection_hitl,
        {WAIT_CONTROLLER: WAIT_CONTROLLER, WAIT_CHANNELS: WAIT_CHANNELS},
    )
    # channel_selection: non-interleaved auto-sets channels=1 → topology_specific_intake
    #                    interleaved → WAIT_CHANNELS for designer to pick 2/3/4
    g.add_edge(WAIT_CHANNELS, END)
    def route_channel_selection(state):
        return state.get("pending_step", WAIT_CHANNELS)
    g.add_conditional_edges(
        "channel_selection", route_channel_selection,
        {"topology_specific_intake": "topology_specific_intake", WAIT_CHANNELS: WAIT_CHANNELS},
    )
    g.add_conditional_edges(
        "channel_selection_hitl", route_after_channel_selection_hitl,
        {"topology_specific_intake": "topology_specific_intake", WAIT_CHANNELS: WAIT_CHANNELS},
    )
