...
def analyze(spec, core, mat, wind, lim, cooling, airflow, rho, alpha, measured, fields, coreLoss_prov="analytic",
    ...
    # Sub-skin stranding only applies to a LITZ design (n_strands > 1). A solid /
    # single-strand magnet wire legitimately exceeds 2*delta when the current is
    # low-frequency dominated — its small AC excess is captured by Rac/Rdc, so it
    # must NOT hard-fail the verdict. Litz designs still require strand < 2*delta.
    skin_ok = (wind.n_strands <= 1) or (wind.strand_d_mm / 1000.0 < 2 * delta)
    ...
    asserts = {
        ...
        "skin_depth": (skin_ok,
                       (f"solid conductor ({wind.strand_d_mm} mm) — sub-skin stranding N/A "
                        f"(2*delta {2*delta*1e3:.3f} mm)") if wind.n_strands <= 1
                       else f"litz strand {wind.strand_d_mm} mm < 2*delta {2*delta*1e3:.3f} mm"),
